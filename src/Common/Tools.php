...
namespace NFePHP\NFSeBHISS\Common;
...
class Tools
{
    ...
    public function send($message, $operation)
    {
        ...
        $response = $this->extractContentFromResponse($response, $operation);
        return(array('operation'=>$operation, 'xmlInput'=>$content, 'xmlOutput'=>$response, 'response'=>json_decode(json_encode(simplexml_load_string($response)), true)));
    }
